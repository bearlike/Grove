...
* line 2 — branch (bold $ref teal) + agent (bold $ref-info cyan) + status
  label (bold + status color), optionally followed by a bold-error
  `init failed` badge when init_status is FAILED.
...
class WorkspaceCard(ListItem):
    ...
    def _initial_body(self) -> Text:
        text = _render_card(self._state, dark=self._dark(), pulse_frame=self.pulse_frame)
