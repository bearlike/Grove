...
class GroveMcpServer:
    ...
    def _register_resources(self) -> None:
        """Expose exact local bundled help to resource-aware MCP clients."""

        @self._mcp.resource(
            "grove://skills/{name}",
            name="grove_skill",
            title="Grove bundled skill",
            description="Complete installed Grove SKILL.md text by name.",
            mime_type="text/markdown",
        )
        def bundled_skill_resource(name: str) -> str:
    ...
    def _register_tools(self) -> None:
        ...
        registrations = (
            ...
            (t.list_watches, "grove_list_watches", False),
            ...
            (t.register_watch, "grove_register_watch", True),
            (t.cancel_watch, "grove_cancel_watch", True),
        ...
        for fn, name, mutates in registrations:
            ...
            # encode its Image wrapper rather than Pydantic serializing it.
            self._mcp.add_tool(
                fn,
                name=name,
                structured_output=False if name == "grove_read_diagram_preview" else None,
            )
