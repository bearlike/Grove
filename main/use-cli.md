...
root all show up in one place. It works from any directory inside the project,
including from inside a worktree. Everything here is read-only.

`grove sessions list` prints every session, newest first. Each row shows the
short session id, the agent, the owning workspace (when Grove knows it), the
live state, the human turn count, and the session title or latest prompt.

```bash
grove sessions list                       # table, newest first
grove sessions list --json                # full metadata for jq
grove sessions list --since 2d --limit 5  # recent sessions only
grove sessions list --workspace auth      # one workspace's sessions
grove sessions list --agent claude_code   # one agent kind
```

`--since` accepts a relative window (`30m`, `6h`, `2d`, `1w`) or an ISO date.
`--workspace` matches a workspace id prefix or a title substring.

`grove sessions show <id>` prints the conversation as turns: each of your
prompts followed by the assistant replies and tool calls it produced. Any
unique id prefix works. Use `--last` to read just the tail of a long session.

```bash
grove sessions show 7b3f2c1a            # the whole conversation
grove sessions show 7b3f2c1a --last 3   # only the last 3 turns
grove sessions show 7b3f2c1a --json     # structured turns
```

`grove sessions dump <id>` emits the session's raw native records (for Claude
Code, the transcript JSONL plus any sub-agent files). The default is one JSON
object listing each file and its parsed records. `--jsonl` streams the
